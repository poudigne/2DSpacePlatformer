...
public class MegaGrab : MonoBehaviour
{
	...
	void DoGrabTGA()
	{
		if ( InitGrab(Screen.width, Screen.height, AASamples) )
		{
			...
			if ( doingSeqGrab )
			{
				fname = epath + Path + SaveName + " " + framenumber;
			}
			else
				fname = epath + Path + SaveName + " " + (width * ResUpscale) + "x" + (height * ResUpscale) + " " + System.DateTime.Now.ToString(Format);
	...
	void DoGrabJPG()
	{
		if ( InitGrab(Screen.width, Screen.height, AASamples) )
		{
			...
			for ( int y = 0; y < ResUpscale; y++ )
			{
				...
				for ( int x = 0; x < ResUpscale; x++ )
				{
					//Debug.Log("Doing Grab " + count + " of " + total);
					count++;
					float xo = (float)x / (float)ResUpscale;
	...
	void LateUpdate()
	{
		...
		if ( Input.GetKeyDown(GrabKey) || doingSeqGrab )
		{
