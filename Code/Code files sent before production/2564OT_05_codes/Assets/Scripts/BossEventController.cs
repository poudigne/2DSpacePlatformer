...
public class BossEventController : MonoBehaviour
{
	...
	public GameObject notActiveNode = null;
	...
	void Start()
	{
		transform.position = notActiveNode.transform.position;
	...
	void Update()
	{
		switch(currentEvent)
		{
			...
			case bossEvents.fallingToNode:
				...
				{
					// Fallspeed here is negtive, so the object moves downwards
               ...
               case bossEvents.jumpingOffPlatform:
                    ...
                    {
						// Fallspeed is positive here, causing the object to move upwards
						transform.Translate(new Vector3(0f, fallSpeed * Time.deltaTime, 0f));
     ...
     void killBoss()
     {
		...
		// Get the enemy boss position
		Vector3 enemyPos = transform.position;
         
		// Create a new vector that is in front of the enemyboss
		Vector3 particlePosition = new Vector3(enemyPos.x,enemyPos.y,enemyPos.z + 1.0f);
         
		// Respoition Reposition the particle emitter at this new position
		deathFxParticle.transform.position = particlePosition;
              
		// Call the bossDied event and give it a score of 25.1000
		if(bossDied != null)
			bossDied(1000);
